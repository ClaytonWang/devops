...
aws cloudformation deploy \
    ...
    --stack-name damo-knife-taxonomy-$usedEnviron --parameter-overrides UsedEnviron=$usedEnviron\
...
then
  ...
  fe_S3=$(aws cloudformation describe-stacks \
            --stack-name damo-knife-frontend-$usedEnviron \
            --query "Stacks[0].Outputs[?OutputKey=='FrontendS3Bucket'].OutputValue" \
            --output text)

  tax_S3=$(aws cloudformation describe-stacks \
          --stack-name damo-knife-taxonomy-$usedEnviron \
          --query "Stacks[0].Outputs[?OutputKey=='TaxonomyS3Bucket'].OutputValue" \
          --output text)

  aws s3 mv "$fe_S3/current" "$tax_S3" --exclude '*' --include "*taxonomy.json" --recursive
  aws s3 mv "$fe_S3/current" "$tax_S3" --exclude '*' --include "*taxonomy.json.gz"  \
